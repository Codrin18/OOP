...
{
	QListWidgetItem* item = this->repoList->takeItem(repoList->currentRow());
	QString text = item->text();
	string text2 = text.toStdString();
	int pos = text2.find("http");
	string l;
	for (int i = pos; i < text2.size(); ++i)
	{
		l += text2[i];
	}
	QString link = QString::fromStdString(l);
